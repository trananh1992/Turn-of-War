...
public final class BitmapChanger {
	...
	public static Bitmap changeColour(final Integer itsaflag, final Bitmap templateBitmap, final Integer originalColour, final Integer replacementColour) {
		...
		replacement.getPixels(allpixels, 0, replacement.getWidth(), 0, 0, replacement.getWidth(), replacement.getHeight());
		...
		replacement = new ColourSwap().Swap(allPxs, allpixels, replacementColour, replacement, itsaflag);

		return replacement;
	}

	public static Bitmap combineMap(final GameMap map, final Integer tilesize, final Map<String, Map<String, Bitmap>> graphics) {
		Bitmap result = null;
		...
		for (int i = 0; i < map.getWidth(); ++i) {
			for (int j = 0; j < map.getHeight(); j++) {
				...
				RectF dest = new RectF(tilesize * i, tilesize * j, tilesize * i + tilesize, tilesize * j + tilesize);
...
//					String A = "yolo";
//					Log.v(A, "n " + n + " getbuilding" + (graphics.get("Buildings").get(n)) + " " + "dev" + n.toLowerCase(Locale.ENGLISH));

					if (((graphics.get("Buildings").get(n))) != null) { //swap texture to dev texture if missing
						combined.drawBitmap(graphics.get("Buildings").get(n), null, dest, null);
					} else {
						String name = "dev" + n.toLowerCase(Locale.ENGLISH); // terrible bugs otherwise
	...
	private static void drawBorder(final GameMap map, final Canvas canvas, final Position currentField, final RectF dest, final Map<String, Map<String, Bitmap>> graphics) {
		...
		Func<String, Void> drawer = new Func<String, Void>() {
			public Void apply(String sprite) {
				canvas.drawBitmap(graphics.get("Fields").get(sprite), null, dest, null);
