...
public final class Logic {
	...
	public List<Position> findValidFieldsNextToUnit(final GameMap map, final Unit attackerUnit, final Unit targetUnit) {
		...
		while (iter.hasNext()) {
			...
			if ((gf.hostsUnit() && !gf.getUnit().equals(attackerUnit)) || !dests.contains(p)) {
				iter.remove();
			}
		}
	...
	public List<Position> findPath(final GameMap map, final Unit unit, final Position destination) {
		return AStar(map, unit, destination);
	}

	public Integer calculateMovementCost(final GameMap map, final Unit unit, final List<Position> path) {
	...
	public List<Position> destinations(final GameMap map, final Unit unit) {
		...
		while (next.size() != 0) {
			...
			for (Node n : next) {
				...
				if (map.getField(n.getPosition()).doesAcceptUnit(unit)) {
					// if (map.getField(n.getPosition()).hostsUnit() && !(map.getField(n.getPosition()).getUnit().getName().equals("Submarine")) && !(map.getField(n.getPosition()).getUnit().getName().equals("Stealth"))) {
					if ((map.getField(n.getPosition()).hostsUnit() && !(map.getField(n.getPosition()).getUnit().getName().equals("Submarine")) && !(map.getField(n.getPosition()).getUnit().getName().equals("Stealth"))) || (map.getField(n.getPosition()).hostsUnit() && ((unit.getName().equals("Destroyer") || unit.getName().equals("Submarine"))) && map.getField(n.getPosition()).getUnit().getName().equals("Submarine")) || (map.getField(n.getPosition()).hostsUnit() && ((unit.getName().equals("Fighter") || unit.getName().equals("Stealth"))) && map.getField(n.getPosition()).getUnit().getName().equals("Stealth"))) {
						Player op = map.getField(n.getPosition()).getUnit().getOwner(); // pass through subs and stealths
						// || ((!unit.getName().equals("Stealth") || !unit.getName().equals("Fighter")) && !map.getField(n.getPosition()).getUnit().getName().equals("Stealth"))
		...
		for (Position p : reachable) {
			if (map.getField(p).canBeStoppedOn()) {
				...
				// if (getMovementCost(map, unit, p) >= 3) { ///find a type of terrain by its movement cost eg trees = 3
				shown.add(p);

				if (!(unit.isFlying() || unit.isBoat())) { // only need to do this for non boats and planes
					Double totalCost = 0.0;
					...
					if (totalCost > unit.getMaxMovement()) {
						shown.remove(p); // remove places that cannot be reached (it would be better to not add them to begin)
					}
				}
			}
		}
	...
	public Pair<Double, Double> calculateDamageFrom(final GameMap map, final Unit attacker, final Unit defender, final Position position) {
	...
	public Pair<Double, Double> calculateDamage(final GameMap map, final Unit attacker, final Unit defender) {
	...
	public Double calculateRawDamage(final GameMap map, final Unit attacker, final Unit defender) {
		...
		if (defender.isFlying()) {
			...
			fieldDefense = defenderField.getDefenseModifier(); // original attackers defense mod
		}
		...
		// String A = "yolo";
		// Log.v(A, "field defense of target " + fieldDefense + " " + (1 - ((attacker.getHealth() * fieldDefense) / 100)));

		Double finalDamage = ((damage * (Math.ceil(attacker.getHealth()) / 10)) * (1 - ((defender.getHealth() * fieldDefense) / 100))) / 10;

		Random r = new Random();
		int min = 0;
		int max = 4;
		double crit = (r.nextDouble() * ((max - min) + min)); // if crit ceil the damage
		...
		// String A = "yolo";
		// Log.v(A, "damage to defender " + " " + " " + finalDamage + " atkhp" + attacker.getHealth() + " " + defender.getHealth() + " defhp");
		return attacker.getHealth() > 0.0 ? finalDamage : 0.0;
	}

	public Double calculateCounterDamage(final GameMap map, final Unit attacker, final Unit defender) {
		...
		return calculateTheoreticalCounterDamage(map, defender, attacker, defenderHealth);
	}

	private Double calculateTheoreticalCounterDamage(final GameMap map, final Unit attacker, final Unit defender, final Double atkHealth) {
		GameField attackerField = map.getField(defender.getPosition()); // this is swapped to defender
		...
		if (defender.isFlying()) {
			...
			fieldDefense = attackerField.getDefenseModifier(); // original attackers defense mod
		}
		int damage = 0;
		damage = new UnitDamages().CalculateDamage(attacker, defender);

		// String A = "yolo";
		// Log.v(A, "field defense of initial attacker " + fieldDefense + " " + (1 - ((attacker.getHealth() * fieldDefense) / 100)));

		double finalDamage;
		if (!defender.isRanged() && !attacker.isRanged()) {
			finalDamage = ((damage * (atkHealth / 10)) * (1 - ((attacker.getHealth() * fieldDefense) / 100))) / 10;
		} else {
			finalDamage = 0;
		}
		...
		// String A = "yolo";
		// Log.v(A, " damage to attacker " + finalDamage + " atkhp" + attacker.getHealth() + " " + defender.getHealth() + " defhp");
		return attacker.getHealth() > 0 ? finalDamage : 0;
	}

	private List<Position> AStar(final GameMap map, final Unit unit, final Position destination) {
		if (!map.isValidField(destination)) {
			return new ArrayList<Position>(0);
		}

		PriorityQueue<Node> openSet = new PriorityQueue<Node>(10, new AStarComparator());
		Set<Node> closedSet = new HashSet<Node>();

		Node root = new Node(unit.getPosition(), 0.0, 1.0 * getManhattanDistance(unit.getPosition(), destination));
		openSet.add(root);
		...
		while (openSet.size() != 0) {
			...
			for (Position n : getValidNeighbours(map, current.getPosition())) {
				...
				if ((!gf.doesAcceptUnit(unit)) || (gf.hostsUnit() && (!(gf.getUnit().getOwner().equals(unit.getOwner()))) && !((gf.getUnit().getName().equals("Stealth") || (gf.getUnit().getName().equals("Submarine")))))) { // don't consider squares with enemy units in
					continue;
				}
				Node neigh = new Node(n, gf.getMovementModifier(), 1.0 * getManhattanDistance(unit.getPosition(), destination));
	...
	private List<Position> getValidSurroundingPositions(final GameMap map, // Attack directions
			final Position pos) {
		List<Position> positions = getValidNeighbours(map, pos);
		return positions;
	}

	private List<Position> getValidNeighbours(final GameMap map, final Position pos) {
	...
	private List<Position> getValidSurroundingPositionsRanged(final GameMap map, // Attack directions
			final Position pos, String name) {
		List<Position> positions = getValidNeighboursRanged(map, pos, name);
		return positions;
	}

	private List<Position> getValidNeighboursRanged(final GameMap map, final Position pos, String name) {
		...
		for (x = -max; x <= +max; x++) {
			for (y = -max; y <= +max; y++) {
				if (((Math.abs(x) + Math.abs(y)) >= min) && ((Math.abs(x) + Math.abs(y)) <= max)) {
	...
	private Double getMovementCost(final GameMap map, final Unit unit, final Position origin) {
		/* g(x) for search */
		// flying units ignore this; always 1
		Double moveCost = 100.0; // unit wont move if everything isnt set up right
	...
	public Set<Position> getAttackableUnitPositions(final int myGo, final GameMap map, final Unit unit, final Position position) {
			...
			if (map.isValidField(p)) {
				if (myGo == 0 && unit.isRanged) {
					...
					if (map.getField(p).hostsUnit()) {
						...
						boolean hostile = !map.getField(p).getUnit().getOwner().equals(unit.getOwner());
						...
						// ((target.getName().equals("BattleCopter"))||(target.getName().equals("TransportCopter"))) //helicopter
						//

						if (unit.getName().equals("Tank") || unit.getName().equals("HeavyTank")) {
							if (hostile && (!target.isBoat() && !target.isFlying())) { // shoot at all ground units
								atkUnits.add(p);
							}
						} else if (unit.getName().equals("AntiAir") || unit.getName().equals("AntiAirBoat")) {
							if (hostile && !target.getName().equals("Stealth") && target.isFlying()) { // shoot at all air units, not stealth
								atkUnits.add(p);
							}
						} else if (unit.getName().equals("Fighter")) {
							if (hostile && target.isFlying()) { // shoot at all air units
								atkUnits.add(p);
							}
						} else if (unit.getName().equals("Stealth")) {
							if (hostile && !target.getName().equals("Submarine") && !target.getName().equals("MissileSub")) { // shoot at all apart from subs
								atkUnits.add(p);
							}
						...
						} else if (unit.getName().equals("Bomber") || unit.getName().equals("Battleship") || unit.getName().equals("RocketTruck") || unit.getName().equals("MissileSub")) {
							if (hostile && !target.isFlying() && !target.getName().equals("Submarine") && !target.getName().equals("MissileSub")) {
								atkUnits.add(p);
							}
						}
					}
	...
	public Set<Position> getAttackableUnitPositions(int myGo, final GameMap map, final Unit unit) {

		return getAttackableUnitPositions(myGo, map, unit, unit.getPosition());
	}

	private Set<Position> getAttackableFields(final int myGo, final GameMap map, final Unit unit, final Position position) {
		String name = unit.getName(); // sort out ranged/owned/direct range squares
		if (!unit.isRanged()) {
			return new HashSet<Position>(getValidSurroundingPositions(map, position));
		} else {
			return new HashSet<Position>(getValidSurroundingPositionsRanged(map, position, name));
		}
	}
	...
	private Integer getManhattanDistance(final Position origin, final Position destination) {
		/* h(x) */
	...
	private Pair<Integer, Integer> getDistanceAway(final Position origin, final Position destination) {
