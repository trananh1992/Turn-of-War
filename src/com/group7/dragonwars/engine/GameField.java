...
public final class GameField extends DrawableMapObject {
	...
	public Boolean doesAcceptUnit(Unit unit) {
		...
		if (this.flightOnly && unit.isBoat() || unit.isFlying()) { //match units to their correct terrains
			this.accessible = true;
		} else {
			canStep = false;
			this.accessible = false;
		}
		if (!this.flightOnly && !unit.isBoat() && !unit.isFlying() && !this.getName().equals("Mountain") && !this.getName().equals("River")) { //land units cant move on mountain and rivers
			this.accessible = true;
		}
		
		if ((this.getName().equals("Mountain") || this.getName().equals("River")) && (unit.getMobility().equals("i")||unit.getMobility().equals("m"))) { //infantry and mechs can go on mountains and rivers
			this.accessible = true;
		}
		
		if (this.hostsBuilding() && this.flightOnly && !unit.isFlying()) { //allow ports to be stepped on by land units (test for a building in the sea)
			this.accessible = true;
		}
	...
	public GameField(final String fieldName, final Double movementModifier, final Double attackModifier, final Double defenseModifier, final Boolean accessible, final Boolean flightOnly, final Boolean boatOnly, final String spriteLocation, final String spriteDir, final String spritePack) {
