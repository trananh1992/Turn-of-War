...
public final class GameView extends SurfaceView implements SurfaceHolder.Callback, OnGestureListener, OnDoubleTapListener, DialogInterface.OnClickListener, OnClickListener {
	...
	private Map<String, Map<String, Bitmap>> graphics = new HashMap<String, Map<String, Bitmap>>();
	...
	private void initialiseGraphics() {
		final int NEUTRAL_FLAG_COLOUR = Color.rgb(241, 245, 252);// -6513508; // teamcolour //Color.rgb(156, 156, 156) //-6513508
		...
		putResource("Misc", "devairfac", "drawable", "com.group7.dragonwars", "devairfac"); //the buildings to swap
		putResource("Misc", "devlandfac", "drawable", "com.group7.dragonwars", "devlandfac");
		putResource("Misc", "devseafac", "drawable", "com.group7.dragonwars", "devseafac");
		putResource("Misc", "devoilfield", "drawable", "com.group7.dragonwars", "devoilfield");
		putResource("Misc", "devoilrig", "drawable", "com.group7.dragonwars", "devoilrig");
		putResource("Misc", "devhq", "drawable", "com.group7.dragonwars", "devhq");
		putResource("Misc", "flag", "drawable", "com.group7.dragonwars", "flag");
		putResource("Misc", "stealthed", "drawable", "com.group7.dragonwars", "stealthed");
		

		/* Load selector and highlighters */
		putResource("Highlighters", "selector", "drawable", "com.group7.dragonwars", "selector");
		putResource("Highlighters", "highlight", "drawable", "com.group7.dragonwars", "highlighter");
		putResource("Highlighters", "path_highlight", "drawable", "com.group7.dragonwars", "pathHighlighter");
		putResource("Highlighters", "attack_highlight", "drawable", "com.group7.dragonwars", "attackHighlighter");
		putResource("Highlighters", "target_highlight", "drawable", "com.group7.dragonwars", "targetHighlighter");
		loadBorders();
		...
		for (Player p : state.getPlayers()) {
			...
			Bitmap colourFlag = BitmapChanger.changeColour(itsaflag, flagBitmap, NEUTRAL_FLAG_COLOUR, p.getColour());
			p.setFlag(colourFlag);
			itsaflag = 0;
			/* All possible units */
			Map<String, Bitmap> personalUnits = new HashMap<String, Bitmap>();
			for (Map.Entry<String, Bitmap> uGfx : graphics.get("Units").entrySet()) {

				Bitmap uBmap = uGfx.getValue();

				if (uBmap == null) {  //this if statement block does the dev textures if originals are missing
					String original = uGfx.getKey();
					String trimmed = "dev" + original.toLowerCase(Locale.ENGLISH); // terrible bugs otherwise
					if (uGfx.getKey().equals(original)) {
						putResource("Misc", trimmed, "drawable", "com.group7.dragonwars", trimmed);
						uBmap = graphics.get("Misc").get(trimmed);
						itsaflag = 1; // dont flip
						Bitmap personal = BitmapChanger.changeColour(itsaflag, uBmap, NEUTRAL_FLAG_COLOUR, p.getColour());
						personalUnits.remove(original);
						personalUnits.put(uGfx.getKey(), personal);
					}
				}

				else if ((!state.getCurrentPlayer().equals(p)) && (uGfx.getKey().equals("Stealth") || uGfx.getKey().equals("Submarine"))) {
	...
	private void stealth() { // slim clone of initialise graphics()
		...
		for (Player p : state.getPlayers()) {
			...
			for (Map.Entry<String, Bitmap> uGfx : graphics.get("Units").entrySet()) {
				...
				if (uBmap == null) {  //this if statement does the dev textures if originals are missing
					String original = uGfx.getKey();
					String trimmed = "dev" + original.toLowerCase(Locale.ENGLISH); // terrible bugs otherwise
					if (uGfx.getKey().equals(original)) {
						putResource("Misc", trimmed, "drawable", "com.group7.dragonwars", trimmed);
				...
				if ((!state.getCurrentPlayer().equals(p)) && (uGfx.getKey().equals("Stealth") || uGfx.getKey().equals("Submarine"))) {
					personalUnits.put(uGfx.getKey(), stealthed);
				} else {
					Bitmap personal = BitmapChanger.changeColour(itsaflag, uBmap, NEUTRAL_FLAG_COLOUR, p.getColour());
					personalUnits.put(uGfx.getKey(), personal);
	...
	private <T extends DrawableMapObject> void putGroup(final String category, final Map<Character, T> objMap) {
		graphics.put(category, new HashMap<String, Bitmap>());

		for (Map.Entry<Character, T> ent : objMap.entrySet()) {
			T f = ent.getValue();
			putResource(category, f.getSpriteLocation(), f.getSpriteDir(), f.getSpritePack(), f.getName());
		}
	}

	private void putResource(final String category, final String resName, final String resDir, final String resPack, final String regName) {
		Integer resourceID = getResources().getIdentifier(resName, resDir, resPack);
		Bitmap bMap = BitmapFactory.decodeResource(context.getResources(), resourceID);
	...
	private void loadField(final String resName, final String regName) {
		putResource("Fields", resName, "drawable", "com.group7.dragonwars", regName);
	}
	...
	private void loadBorders() {
		...
		for (String b : borderList) {
			for (Integer i = 1; i <= 4; i++) {
				loadField(String.format("border_%s_%d", b, i), String.format("border %s %d", b, i));
				loadField(String.format("corner_%s_%d", b, i), String.format("corner %s %d", b, i));
				loadField(String.format("fullcorner_%s_%d", b, i), String.format("fullcorner %s %d", b, i));
			}
		}
	}
	...
	public void doDraw(final Canvas canvas) {
		...
		canvas.drawBitmap(map.getImage(), scrollOffset.getX(), scrollOffset.getY(), null);

		for (int i = 0; i < map.getWidth(); ++i) {
			for (int j = 0; j < map.getHeight(); j++) {
				GameField gf = map.getField(i, j);
				RectF dest = getSquare(tilesize * i + scrollOffset.getX(), tilesize * j + scrollOffset.getY(), tilesize);

				if (gf.hostsBuilding()) {
					Player owner = gf.getBuilding().getOwner();

					/* TODO proper Gaia handling */
					Integer remainingHp = gf.getBuilding().getRemainingCaptureTime();
					if (owner.getName().equals("Gaia")) {
						canvas.drawBitmap(graphics.get("Misc").get("flag"), null, dest, null);
					} else {
					...
					if (remainingHp != 20) { // show capture text
						...
						canvas.drawText(captureText, dest.left + 115, dest.top + 40, capOutlinePaint);
						canvas.drawText(captureText, dest.left + 115, dest.top + 40, capPaint);
					}
				}
				...
				if (gf.hostsUnit()) {
					...
					if (owner.getName().equals("Gaia")) {
						...
						if (unit.isBoat()) { // raise boats
							dest = getSquare(tilesize * i + scrollOffset.getX(), tilesize * j + scrollOffset.getY() - 30, tilesize);
						}
						...
						String healthText;
						if (unit.getHealth() > 1) {
							healthText = decformat.format(Math.round(unit.getHealth()));//
						} else {
							healthText = decformat.format(ceilHealth);
						}

						double fullHpCheck = Double.parseDouble(healthText);
						if (unit.hasFinishedTurn() == true && unit.getOwner() == state.getCurrentPlayer()) {
							canvas.drawText("x", dest.left + 40, dest.bottom - 2, movedOutlinePaint);
							canvas.drawText("x", dest.left + 40, dest.bottom - 2, movedPaint);

						}
						if (fullHpCheck != 10) {
							if ((unit.getOwner() != state.getCurrentPlayer()) && (unit.getName().equals("Stealth") || (unit.getName().equals("Submarine")))) {

							} else {
								canvas.drawText(healthText, dest.left + 115, dest.bottom - 1, unitHealthOutlinePaint);
								canvas.drawText(healthText, dest.left + 115, dest.bottom - 1, unitHealthPaint);
							}
						}
					}
				}
		...
		while (iter.hasNext()) {
			...
			if (currentUnit == null || currentUnit.isDead()) {
				// MediaPlayer mp = MediaPlayer.create(context, R.raw.cannhvy1); // death sound
		...
		RectF selectDest = getSquare(tilesize * selected.getX() + scrollOffset.getX(), tilesize * selected.getY() + scrollOffset.getY(), tilesize);
		canvas.drawBitmap(graphics.get("Highlighters").get("selector"), null, selectDest, null);
		...
		drawCornerBox(canvas, false, true, player.getName() + " - $" + player.getGoldAmount() + " Mil" + "\nFPS: " + fpsS, true, playerPaint);

		state.endFrame();
	}

	public void highlightPositions(final Canvas canvas, final Collection<Position> positions, final String highlighter) { // /////////////////////////
		for (Position pos : positions) {
			RectF dest = getSquare(tilesize * pos.getX() + scrollOffset.getX(), tilesize * pos.getY() + scrollOffset.getY(), tilesize);
			canvas.drawBitmap(graphics.get("Highlighters").get(highlighter), null, dest, null);
		}
	}
	...
	public void drawInfoBox(final Canvas canvas, final Unit unit, final GameField field, final boolean left) {
		...
		if (unit != null && !unit.isRanged() && !unit.getName().equals("Stealth") && !unit.getName().equals("Submarine")) {
			info += unit.getInfo();

		} else if (unit != null && unit.getName().equals("Stealth") && (unit.getOwner() == state.getCurrentPlayer())) {
			info += unit.getInfo();
		} else if (unit != null && unit.getName().equals("Submarine") && (unit.getOwner() == state.getCurrentPlayer())) {
			info += unit.getInfo();
	...
	public void drawCornerBox(final Canvas canvas, final boolean left, final boolean top, final String text) {
		drawCornerBox(canvas, left, top, text, false, null);
	}

	public void drawCornerBox(final Canvas canvas, final boolean left, final boolean top, final String text, final boolean box, final Paint boxPaint) {
		...
		cornerBoxTextPaint.getTextBounds(longestLine, 0, longestLine.length(), bounds);
		Integer boxWidth = bounds.width(); // Might have to Math.ceil first
		Integer boxHeight = ss.length * bounds.height();

		Rect backRect = new Rect(left ? 0 : canvas.getWidth() - boxWidth, top ? 0 : canvas.getHeight() - boxHeight, left ? boxWidth : canvas.getWidth(), top ? boxHeight : canvas.getHeight());
		float radius = 5f;
		RectF backRectF = new RectF(backRect.left - radius - (box && !left ? radius + boxHeight : 0), backRect.top - radius, backRect.right + radius + (box && left ? radius + boxHeight : 0), backRect.bottom + radius);
		canvas.drawRoundRect(backRectF, 5f, 5f, cornerBoxBackPaint);

		if (box) {
			canvas.drawRect(new RectF(left ? backRect.right + radius : backRect.left - radius - boxHeight, backRect.top, left ? backRect.right + radius + boxHeight : backRect.left - radius, backRect.top + boxHeight), boxPaint);
		}

		for (Integer i = 0; i < ss.length; ++i) {
			canvas.drawText(ss[i], 0, ss[i].length(), left ? backRect.left : backRect.right, backRect.top + (bounds.height() * (i + 1)), cornerBoxTextPaint);
		}
	}
	...
	public boolean onSingleTapConfirmed(final MotionEvent event) {
		...
		if (this.map.isValidField(touchX, touchY)) {
			...
			if (!state.getCurrentPlayer().isAi()) {
				...
				if (!state.getAttackables().contains(newselected)) { // allow units to move to squares which are highlighted by attack if (!state.getAttackables().contains(newselected) || !(state.getAttackables().contains(newselected) && !(map.getField(selected).hostsUnit()))) {
					if (map.getField(selected).hostsUnit()) {
						// Log.d(TAG, "A unit is selected!");
						/*
						 * If the user currently has a unit selected and selects a field that this unit could move to (and the unit has not finished it's turn)
						 */
						...
						if (unit.getOwner().equals(state.getCurrentPlayer()) && !unit.hasFinishedTurn() && (!map.getField(newselected).hostsUnit() || selected.equals(newselected))) {
							List<Position> unitDestinations = state.getUnitDestinations(selectedField);
							if (unit.hasMoved() && (((unit.getName() != "Infantry") || (unit.getName() != "Mech")) && !newselectedField.hostsBuilding() && !selectedField.hostsBuilding())) { // infantry on building dont get marked as moved
								unit.setFinishedTurn(true);
							} else {// if the building belongs to the player then mark the infantry as moved anyway
								if (unit.hasMoved() && (selectedField.hostsBuilding())) {
									if (selectedField.getBuilding().getOwner().equals(state.getCurrentPlayer())) {
										unit.setFinishedTurn(true);
									}
								}
							}

							if (unitDestinations.contains(newselected) || selected.equals(newselected)) {
					...
					} else if (!newselectedField.hostsUnit() && newselectedField.hostsBuilding()) {
						state.setPath(null);
						...
						if (building.getOwner().equals(state.getCurrentPlayer()) && building.canProduceUnits()) {
							AlertDialog.Builder buildmenuBuilder = new AlertDialog.Builder(this.getContext());
							String menuTitle = "";
							...
							for (int i = 0; i < units.size(); ++i) {
								Unit cU = units.get(i);
								buildableNames[i] = String.format("%s   $%s Million", cU.toString(), cU.getProductionCost());
							}
					...
					if (state.getCurrentPlayer().equals(attacker.getOwner())) { // if (state.getCurrentPlayer().equals(attacker.getOwner()) && defender != null) { you can only attack with your own units on your own go and there has to be a defending unit
						Log.d(TAG, "attack(!): " + attacker + " attacks " + defender);
	...
	public void onClick(final DialogInterface dialog, final int which) {
		...
		if (field.hostsBuilding() && (field.getBuilding().getProducibleUnits().size() > which) && state.getCurrentPlayer().equals(field.getBuilding().getOwner())) {
			Unit unit = map.getField(selected).getBuilding().getProducibleUnits().get(which);
			Log.d(TAG, "building a " + unit);
			Boolean result = state.produceUnit(map.getField(selected), unit);

			if (!result) {
				alertMessage(String.format("Insufficient funds to buy %s", unit, unit.getProductionCost()));
			}
	...
	private void alertMessage(final String text) {
		new AlertDialog.Builder(context).setMessage(text).setPositiveButton("OK", null).show();
	}
	...
	public void onClick(final View arg0) {
		...
		try {
			...
			Toast.makeText(context, String.format("%s's turn!", state.getCurrentPlayer()), Toast.LENGTH_SHORT).show();
		} catch (GameFinishedException e) {
			Player winner = e.getWinner();
			Toast.makeText(context, String.format("%s has won the game!", winner.getName()), Toast.LENGTH_SHORT).show();
