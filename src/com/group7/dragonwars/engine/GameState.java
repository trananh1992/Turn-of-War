...
public final class GameState {
	...
	public GameState(final GameMap map, final Logic logic, final List<Player> players, final GameView gv) {
		this.map = map;
		this.logic = logic;
	...
	public void attack(final Unit attacker, final Unit defender) {
		Set<Position> attackable = logic.getAttackableUnitPositions(0, map, attacker);
	...
	public Boolean move(final Unit unit, final Position destination) {
		/*
		 * We are assuming that the destination was already checked to be within this unit's reach
		 */
		...
		String CurrPlayer = String.valueOf(getCurrentPlayer()); // reset hp if moved unit off building
	...
	private Boolean removeUnitIfDead(final Unit unit) {
		if (unit.isDead()) {
			...
			String CurrPlayer = String.valueOf(getCurrentPlayer()); // reset hp if unit is dead
			String CurrString = CurrPlayer;
			movedOffBuildingCaptureCounters(CurrString);
			return true;
		}
	...
	private void updateBuildingRepairUnits(String currString) { // healing mechanic
		for (GameField gf : map) {
			...
			if (gf.hostsUnit()) {
				...
				if (unit.getHealth() < unit.getMaxHealth() && OwnerInt == currInt && unit.getOwner() == gf.getBuilding().getOwner()) {
					if ((gf.getBuilding().getName().equals("seafac") && unit.isBoat()) || (gf.getBuilding().getName().equals("airfac") && unit.isFlying()) || ((gf.getBuilding().getName().equals("landfac") || gf.getBuilding().getName().equals("city") || gf.getBuilding().getName().equals("hq")) && !unit.isFlying() && !unit.isBoat())) { // units only repair on their respective factories + land repair on cities
						// also need to check for unit type and building type in conditions
						if (healCost * 2 <= playerCash) {
							healAmount = 2;
						} else if (healCost <= playerCash && healCost * 2 >= playerCash) {
							healAmount = 1;
						} else {
							healAmount = 0;
						}

						// Log.v(tagasdafs, "lol" + unit.getHealth() + " " + healAmount);
						unit.restoreHealth((double) healAmount);
						unit.getOwner().setGoldAmount((playerCash - (healCost * healAmount)));
					}
				}
			}
		}
	...
	private void updateBuildingCaptureCounters(String currString) { // this is for making capture counters adjust after each turn in realtime
		for (GameField gf : map) {
			/* No building. */
			if (!gf.hostsBuilding()) {
				continue;
			...
			if (gf.hostsUnit() && (gf.getUnit().getName().equals("Tank"))) {
				...
				int OwnerInt = Integer.parseInt(OwnerArr, 10);
				// String A = "yolo";
				// Log.v(A, "lol" + unit.getOwner() + " £ " + currInt + " " + OwnerInt + " " + gf.hostsUnit());
				...
				if (OwnerInt == currInt) {
					...
					if (unit.getOwner().equals(b.getLastCapturer()) && unit.getOwner() != gf.getBuilding().getOwner() && unit.hasFinishedTurn() == false) { // just add &&unit.getType() == ""
						whoCappedThatLastHQ = b.reduceCaptureTime(turnReduce);
						continue;
					} else {
						// b.resetCaptureTime();
						b.setLastCapturer(unit.getOwner());
						if (unit.getOwner() == b.getLastCapturer() && unit.getOwner() != gf.getBuilding().getOwner() && unit.hasFinishedTurn() == false) { // just add &&unit.getType() == ""
							whoCappedThatLastHQ = b.reduceCaptureTime(turnReduce);
						}
					}
				}
	...
	private void movedOffBuildingCaptureCounters(String currString) { // reset HP when moved off building or killed
		for (GameField gf : map) {
			/* No building. */
			if (!gf.hostsBuilding()) {
				continue;
	...
	public void nextPlayer() throws GameFinishedException {
		...
		updateBuildingCaptureCounters(CurrString); // deal with those pesky counters
		...
		String nextPlayerComingUp = (players.get(playerIndex).toString()); // these two lines make repairs happen at start of round instead of end
		...
		while (iter.hasNext()) {
			...
			if (p.hasLost()) { // what to do when player dies
				...
				for (Building b : p.getOwnedBuildings()) {
					for (GameField gf : map) { // this for loop is to reset the counters of formerly capturing units that are now dead
						if (gf.hostsBuilding() && gf.hostsUnit()) {
							Building c = gf.getBuilding();
							if (c.getLastCapturer() == p)
								c.remainingCaptureTime = b.getCaptureDifficulty();
						}
					}
	...
	private void income() {
		for (Player p : players) {
			...
			for (Building b : p.getOwnedBuildings()) {
				...
				// times capturewoth by a multiplier for varying levels of funds, dont remove capture worth - it allows more valuable buildings
			}
