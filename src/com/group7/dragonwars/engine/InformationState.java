...
/**
 * Used by GameState to provide data to GameView in a transparent fashion. All the logic of whether to recalculate something or not will be moved here.
 */
public final class InformationState {
	...
	public List<Position> getPath() {
		if (path == null || lastDestinations == null || lastDestinations.size() == 0) {
	...
	public List<Position> getUnitDestinations(final GameField field) {
		...
		if (field.hostsUnit() && (u.getOwner() != containingState.getCurrentPlayer())) {
			u.resetMovement(); // redraw move distance when its not ur turn}
			u.resetTurnStatistics2();
		}

		// cant show range of enemy stealth or subs
		if ((u.hasFinishedTurn() && (u.getOwner() == containingState.getCurrentPlayer())) || (u.getName().equals("Stealth") && (u.getOwner() != containingState.getCurrentPlayer())) || (u.getName().equals("Submarine") && (u.getOwner() != containingState.getCurrentPlayer()))) {
			lastAttackables = null;
			lastAttackables2 = null;
			...
			if ((lastDestinations == null || lastUnit == null || lastField == null || lastAttackables == null || lastAttackables2 == null) && !u.hasMoved()) {
				lastUnit = u;
				lastField = field;

				int myGo = 1;
				if (u.getOwner() != containingState.getCurrentPlayer() && u.isRanged) {
					// unitDests = logic.destinations(containingState.getMap(), u);
					lastDestinations = null;
					lastAttackables = logic.getAttackableUnitPositions(myGo, containingState.getMap(), u);
					myGo = 0;
					lastAttackables2 = logic.getAttackableUnitPositions(myGo, containingState.getMap(), u);
				} else {
					unitDests = logic.destinations(containingState.getMap(), u);
		...
		if (u.hasMoved() && u.getOwner() == containingState.getCurrentPlayer() && !u.hasFinishedTurn()) {
			path = null;
			int myGo = 1;
			lastAttackables = logic.getAttackableUnitPositions(myGo, containingState.getMap(), u);
			myGo = 0; //display every square
