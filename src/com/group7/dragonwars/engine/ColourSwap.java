...
public class ColourSwap {
	public Bitmap Swap(Integer allPxs, int[] allpixels, Integer replacementColour, Bitmap replacement, Integer itsaflag) {
		...
		replacement.setPixels(allpixels, 0, replacement.getWidth(), 0, 0, replacement.getWidth(), replacement.getHeight()); 
		if ((replacementColour == -571062 || replacementColour == -9256376 || replacementColour == -1804345 || replacementColour == -10586212) && itsaflag != 1) { //spawn odd teams facing right
	...
	private static Bitmap flipIt(Bitmap replacement) {
		...
		Bitmap reflectionImage = Bitmap.createBitmap(replacement, 0, 0, replacement.getWidth(), replacement.getWidth(), matrix, false);
